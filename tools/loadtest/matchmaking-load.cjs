...
async function authToken(i) {
  ...
  if (!res.ok)
    throw new Error(
      `user ${i}: auth ${res.status} (${res.status === 429 ? 'rate-limited — seed users for large runs' : await res.text()})`,
    );
...
(async () => {
  ...
  const sockets = tokens.map((t) =>
    io(`${WS}/mm`, { auth: { token: t }, transports: ['websocket'], reconnection: false }),
  );
  await Promise.all(
    sockets.map(
      (s) =>
  ...
  sockets.forEach((s, i) =>
    s.once('mm:matched', () => {
      matched[i] = true;
      if (joinAt[i]) latencies.push(now() - joinAt[i]);
    }),
  );
  ...
  console.log(`\n── RESULT ──`);
  console.log(
    `  matched:        ${matchedCount}/${N} clients (${((matchedCount / N) * 100).toFixed(0)}%)`,
  );
  console.log(
    `  match latency:  p50=${p(0.5)}ms  p90=${p(0.9)}ms  max=${latencies[latencies.length - 1] ?? 0}ms`,
  );
