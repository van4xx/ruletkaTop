...
const PERKS = [
  { icon: Zap, text: 'Мгновенный коннект' },
  { icon: ShieldCheck, text: 'Безопасно и анонимно' },
  { icon: Sparkles, text: 'Живое общение' },
] as const;
...
export function AuthShell({ children, pitch = 'Встреться с миром в одном клике' }: AuthShellProps) {
  return (
        ...
        >
          ...
          <motion.ul variants={rise} className="mt-6 flex flex-col gap-3">
            {PERKS.map(({ icon: Icon, text }) => (
              <li key={text} className="inline-flex items-center gap-3 text-muted-foreground">
