...
const STATUS_LABEL: Partial<Record<OnlineStatus, string>> = {
  online: 'В сети',
  away: 'Отошёл',
  in_call: 'В звонке',
};
...
export function OnlineFriendsWidget() {
  ...
  return (
    <DashboardCard label="Друзья онлайн">
      <WidgetHeader
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="text-[0.625rem] text-muted-foreground">{STATUS_LABEL[status] ?? 'В сети'}</span>
      </Link>
      ...
      <div className="pointer-events-none absolute inset-x-1.5 top-1.5 flex justify-center gap-1.5 opacity-0 transition-opacity duration-200 group-hover:pointer-events-auto group-hover:opacity-100">
        <Link
          ...
          aria-label={`Написать ${profile.nickname}`}
        >
        ...
        {canCall && (
          <Link
            ...
            aria-label={`Видеозвонок ${profile.nickname}`}
          >
  ...
  return (
    <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-border/70 py-7 text-center">
      ...
      >
        ...
        {hasAny ? 'Открыть друзей' : 'Найти друзей'}
      </Link>
    </div>
  );
}
